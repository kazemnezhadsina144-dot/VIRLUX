...
# vx-0870 — Do not commit partner PII in repo
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0870. Do not commit partner PII in repo (Quarterly hardening. Refactor only if verify stays green.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
