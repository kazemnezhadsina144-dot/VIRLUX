...
# vx-0541 — Validate router.push after login uses next param
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0541. Validate router.push after login uses next param (Next sprint. Ship with evidence in PRIORITY.md.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
