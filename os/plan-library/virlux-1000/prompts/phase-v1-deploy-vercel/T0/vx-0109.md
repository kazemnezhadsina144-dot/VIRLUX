...
# vx-0109 — Fix staging-vercel-deploy.sh trap for vercel.json restore
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0109. Fix staging-vercel-deploy.sh trap for vercel.json restore (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
