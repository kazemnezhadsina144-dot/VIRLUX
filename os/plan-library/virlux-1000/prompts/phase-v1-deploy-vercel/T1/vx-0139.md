...
# vx-0139 — Validate apps/api prisma generate on Vercel build
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0139. Validate apps/api prisma generate on Vercel build (Next sprint. Ship with evidence in PRIORITY.md.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
