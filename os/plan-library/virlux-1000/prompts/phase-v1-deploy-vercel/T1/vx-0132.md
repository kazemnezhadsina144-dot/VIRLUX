...
# vx-0132 — Run staging-supabase-db-url.sh pooler URL
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0132. Run staging-supabase-db-url.sh pooler URL (Next sprint. Ship with evidence in PRIORITY.md.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
