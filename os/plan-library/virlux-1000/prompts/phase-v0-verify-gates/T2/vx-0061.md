...
# vx-0061 — Run npm test -w @virlux/shared; fix failures
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0061. Run npm test -w @virlux/shared; fix failures (Quarterly hardening. Refactor only if verify stays green.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
