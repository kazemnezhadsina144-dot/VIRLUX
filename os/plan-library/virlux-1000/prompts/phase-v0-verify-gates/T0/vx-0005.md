...
# vx-0005 — Harden scripts/staging-smoke.sh API meta checks
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0005. Harden scripts/staging-smoke.sh API meta checks (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
