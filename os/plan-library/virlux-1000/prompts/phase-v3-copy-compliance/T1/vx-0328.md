---
id: vx-0328
phase: phase-v3-copy-compliance
tier: T1
priority: P1
status: backlog
lane: virlux
library: virlux-1000-locked
agent: Auto-VIRLUX-Delivery
slot: 2
generator: scripts/generate-virlux-1000-prompts.py
locked: true
updated_at: 2026-06-06
---

# vx-0328 — Fix CLIENT_COPY usage in apps/app dashboard

**Phase:** `phase-v3-copy-compliance` — public-copy, drift scan, blueprint alignment  
**Tier:** `T1` — Next sprint. Ship with evidence in PRIORITY.md.

## Agent prompt (copy to chat)

```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0328. Fix CLIENT_COPY usage in apps/app dashboard (Next sprint. Ship with evidence in PRIORITY.md.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```

## Task

Fix CLIENT_COPY usage in apps/app dashboard

## Sources (read first)

- `os/agents/auto-virlux-delivery/MEMORY.md`
- `os/plan.json` · `os/plan-library/VIRLUX-PRIORITY.md`
- `packages/shared/src/public-copy.ts` (copy SOT surface)
- `todolist/PUBLIC-BLUEPRINT.md` (gitignored — founder sync only)
- Global pack: `~/.cursor/plans/no-asf-library/REGISTRY.json`

## Verify

```bash
npm run verify:live
```

## Closeout

1. `status: done` in front matter
2. `os/plan-library/VIRLUX-PRIORITY.md` or AUDIT-LOG evidence
3. `bash scripts/sync-virlux-no-asf-registry.sh`
