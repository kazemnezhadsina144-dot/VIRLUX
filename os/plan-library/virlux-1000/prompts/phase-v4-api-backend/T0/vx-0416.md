...
# vx-0416 — Validate Supabase pooler in production DATABASE_URL
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0416. Validate Supabase pooler in production DATABASE_URL (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
