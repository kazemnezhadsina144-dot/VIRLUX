...
# vx-0420 — Add unit test in apps/api for critical route
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0420. Add unit test in apps/api for critical route (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
