...
# vx-0433 — Validate auth login rate limit behavior
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0433. Validate auth login rate limit behavior (Next sprint. Ship with evidence in PRIORITY.md.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
