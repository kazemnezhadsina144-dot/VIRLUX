...
# vx-0474 — Review transactions id route error handling
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0474. Review transactions id route error handling (Quarterly hardening. Refactor only if verify stays green.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
