...
# vx-0756 — Add env guard DEMO_MODE vs production
...
```
PLAN WITH NO ASF — VIRLUX agent prompt vx-0756. Add env guard DEMO_MODE vs production (Quarterly hardening. Refactor only if verify stays green.). Pre-flight: read os/agents/auto-virlux-delivery/MEMORY.md and INCIDENTS.md. Only edit REGISTRY paths or code; tag docs VIRLUX-AUTO-DELIVERY-YYYYMMDD. Post-flight: npm run agent:self-audit and append AUDIT-LOG.
```
